...
class Table(QTableWidget):
    ...
    def extract_data_to_dict(self, before_list, after_list, pin_map, regex, begin_regex):
        ...
        for path_item in path_dict.items():
                ...
                while line is not None and len(line) > 0:
                    ...
                    if testname_dict is not None and compare_testname_dict is not None:
                        ...
                        if pinname_dict is not None:
                            if compare_chip_dict.get(pinname) is None:
            ...
            for temperature_item in after_item.items():
                ...
                # 读取文件
                with open(after_temperature_path, "r", encoding="utf-8") as f:
                    line = f.readline()
                    while line is not None and len(line) > 0:
                        if re.match(begin_regex, line):
                            break
    ...
    def fill_table(self, before_list, after_list, pin_map, regex, begin_regex):
        ...
        try:
            ...
            # 确定列数
            for pin_dict in pin_map.values():
                # 表头
                temp_set = set()
            ...
            for testname_item in pin_map.items():
                ...
                if len(temp_set) > 1:
                    ...
                    for _ in temp_list[1:]:
                        self.setItem(1, col + current_w, self.get_table_item("老炼后"))
                        current_w += 1
                        self.setItem(1, col + current_w, self.get_table_item("变化值"))
                        current_w += 1
                        self.setItem(1, col + current_w, self.get_table_item("变化百分比"))
                        current_w += 1
                ...
                for compare_chip_item in compare_testname_dict.items():
                    ...
                    for pin_item in pin_dict.items():
                        ...
                        self.setItem(row + current_h, col + current_w, self.get_table_item(pinname))
                        current_w += 1
                        ...
                        if before_val:
                            self.setItem(row + current_h, col + current_w,
                            ...
                            if temperature != MIN_SIZE and test_val is not None:
                                ...
                                if before_val:
                                    ...
                                    if lower_bound != "" and diff < lower_bound \
                                        ...
                                        for i in range(-3, 0):
                                            self.item(row + current_h, col + current_w + i) \
                        ...
                        current_h += 1
                    # 芯片编号
                    self.setItem(row, col, self.get_table_item(chip_id, Qt.AlignHCenter | Qt.AlignTop))
    ...
    def get_table_item(self, text, alignment=Qt.AlignHCenter | Qt.AlignVCenter):
